...
// getAllFlights
func (t *FlyNetChaincode) getAllFlights(ctx contractapi.TransactionContextInterface) ([]*Flight, error) {
	flightResultsIterator, err := ctx.GetStub().GetStateByPartialCompositeKey("Flight", []string{})
	if err != nil {
		return nil, fmt.Errorf("Failedto get flights data from ledger: %v", err)
	}
	...
	for flightResultsIterator.HasNext() {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed to get next flight: %v", err)
		}

		flight := new(Flight)
		err = json.Unmarshal(queryResponse.Value, flight)
		if err != nil {
			return nil, fmt.Errorf("Failed to unmarshal flight: %v", err)
		}
...
func (t *FlyNetChaincode) getFlight(ctx contractapi.TransactionContextInterface, flightNr string) (*Flight, error) {
	flightAsBytes, err := ctx.GetStub().GetState(flightNr)
	if err != nil {
		return nil, fmt.Errorf("Failed to read from world state: %v", err)
	}
	if flightAsBytes == nil {
		return nil, fmt.Errorf("Flight %s does not exist", flightNr)
	}
...
func (t *FlyNetChaincode) reserveSeats(ctx contractapi.TransactionContextInterface, flightNr string, number int, customerNames[] string, customerEmail string) (string, error) {
	...
	flight, err := t.getFlight(ctx, flightNr)
	if err != nil {
...
// GetReservation returns the reservation found on the ledger
func (t *FlyNetChaincode) getReservation(ctx contractapi.TransactionContextInterface, reservationNr string) (*Reservation, error) {
	reservationAsBytes, err := ctx.GetStub().GetState(reservationNr)
	if err != nil {
		return nil, fmt.Errorf("Failed to read from world state: %v", err)
	}
	if reservationAsBytes == nil {
		return nil, fmt.Errorf("Reservation %s does not exist", reservationNr)
	}
...
func (t *FlyNetChaincode) bookSeats(ctx contractapi.TransactionContextInterface, reservationNr string) (string, error) {
	...
	reservation, err := t.getReservation(ctx, reservationNr)
	if err != nil {
		return "", fmt.Errorf("Failed getting reservation: %v", err)
	}

	flight, err := t.getFlight(ctx, reservation.FlightNr)
	if err != nil {
		return "", fmt.Errorf("Failed getting flight: %v", err)
	}
...
func (t *FlyNetChaincode) checkIn(ctx contractapi.TransactionContextInterface, reservationNr string, passportIDs []string) (string, []string) {
	return "CheckIn", nil
}

func main() {
	chaincode, err := contractapi.NewChaincode(new(FlyNetChaincode))
	if err != nil {
		fmt.Printf("Error creating flynet chaincode: %s", err.Error())
	}
